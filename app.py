...
<head>
    ...
    <style>
        ...
        .hidden { display: none; }
        #display-text { margin-top: 20px; font-size: 1.2rem; color: #333; line-height: 1.3; text-align: left; }
    </style>
</head>
<body>
    ...
    <script>
        ...
        function speakFastAndFluid(text) {
            // On vide la file d'attente pour éviter les chevauchements
            window.speechSynthesis.cancel();
            
            // On envoie TOUT le texte d'un coup pour éviter les pauses entre les phrases
            const utter = new SpeechSynthesisUtterance(text);
            
            // On force une voix de haute qualité si disponible
            ...
            // Réglages pour une élocution vive et sans temps mort
            utter.rate = 1.35; // Vitesse rapide pour supprimer l'hésitation robotique
            utter.pitch = 1.0;
            utter.volume = 1.0;
        ...
        document.getElementById('submit-name').onclick = function() {
            ...
            if(name.trim() !== "") {
                ...
                // Texte sans trop de ponctuation pour forcer la fluidité
